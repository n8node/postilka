...
func (s *MissionService) Create(ctx context.Context, userID string, r *http.Request, req model.MissionCreateRequest) (*model.Mission, error) {
	...
	_, _ = s.missions.InsertMessage(ctx, model.MissionMessage{
		...
		Role:        "assistant",
		Content:     "Агент запущен. Коротко опишите продукт, аудиторию и что хотите получить — я уточню цель и предложу ход публикаций. Публиковать в сети буду только после вашего разрешения.",
	})
	return created, nil
}
