...
export function WorkspaceMediaPickerModal({
  ...
  const loadBrowse = useCallback(async (targetFolderId: string | null) => {
    ...
    try {
      ...
      setFiles(fileRes.files.filter((f) => matchesKind(f, mediaKind)));
      setFolders(folderRes.folders.map((f) => ({ id: f.id, name: f.name })));
      if (targetFolderId) {
        const crumbs = await fetchFolderBreadcrumbs(targetFolderId);
        setBreadcrumbs([{ id: null, name: "Мои файлы" }, ...crumbs.breadcrumbs]);
      } else {
  ...
  const loadSection = useCallback(async (nextTab: PickerTab) => {
    if (nextTab === "browse") {
    ...
    try {
      ...
      setFiles(res.files.filter((f) => matchesKind(f, mediaKind)));
      setFolders([]);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Не удалось загрузить файлы");
      setFiles([]);
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[min(88vh,720px)] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-border bg-surface shadow-xl"
      ...
      >
        ...
        {tab === "browse" ? (
          <div className="flex flex-wrap items-center gap-1 border-b border-border px-4 py-2 text-[11px]">
            {breadcrumbs.map((crumb, index) => (
              <span key={`${crumb.id ?? "root"}-${index}`} className="flex items-center gap-1">
                {index > 0 ? <ChevronRight size={12} className="text-zinc-300" /> : null}
                <button
                  type="button"
            ...
            <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {folders.map((folder) => (
                ...
                >
                  ...
                  <span className="truncate text-[12px] font-medium">{folder.name}</span>
                </button>
              ))}
            </div>
          ) : null}
