...
export function StoryAreaEditor({
  ...
  return (
    <div className="space-y-4">
      ...
      {areas.some((area) => area.kind === "link" || area.kind === "location") && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
          <p className="font-semibold">Ограничение API Telegram</p>
          <p className="mt-1 leading-snug">
            Из-за известного бага API Telegram стикеры ссылки и геометки могут быть не видны в
            истории, но остаются кликабельными для зрителей.
          </p>
        </div>
      )}
